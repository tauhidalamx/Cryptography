...
def create_tls_context():
    ...
    context.options = ssl.OP_NO_TICKET  # prevent client from requesting session ticket
    return context
...
while True:  # server is always on, listening over TCP socket
    ...
    data = tls_socket.recv(1024).decode()  # receive data from client at socket (socket buffer is 1024 bytes)
